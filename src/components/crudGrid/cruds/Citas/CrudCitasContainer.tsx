...
interface CrudCitasContainerProps<CitaAdminType extends FieldValues> {
    formModalProp: formModalCrudProps
    form: UseFormReturn<CitaCrudData>

    title?: string

    columns: ColumnDef<CitaAdminType>[]
    defaultValues: CitaCrudData

    crudQueries: crudQueries<CitaCrudData>
...
export default function CrudCitasContainer<T extends Record<string, any>>({
    formModalProp,
    form,
    columns,
    defaultValues,
    ...
    const [actualRow, setActualRow] = useState<CitaCrudData | null>(null)
    ...
    const abrirModalDatos = (
        actualRow: CitaCrudData | null = null,
    ...
    const submitingRequest = async (
        row: CitaCrudData,
        cb: (row: CitaCrudData) => Promise<ObjectApiResponse>
    ) => {
    ...
    const deleteAccion = async (row: T) => {
        return submitingRequest(row, deleteRequest)
    }

    const onSubmit = handleSubmit(async (values: CitaCrudData) => {
    ...
    const onCrudActions: onCrudActionsProps<CitaAdminType> = {
        onCreate: () => {
    ...
    const botonIngresar: newActionCrud = {
        ...
        onAction: row => {
            console.log('Ingresar action for row:', row)
        },
    }

    return (
        <>
            <CrudCrudo<CitaAdminType>
                title={title}
                columns={columns}
                fetchData={fetchData}

    ...
    return createPortal(
        <Transition show={isOpen} as={Fragment}>
            <div aria-labelledby='modal-title' role='dialog' aria-modal='true'>
                <div
                    className='fixed inset-0 bg-black bg-opacity-40  z-30 w-screen h-screen flex items-center justify-center p-4'
                    onClick={() => onClose()}
                >
                    {/* Panel del Modal */}
                    <Transition.Child
                    ...
                    >
                        <div
                            className={`bg-background-accent-auto overflow-auto max-h-screen rounded-2xl shadow-xl transition-all sm:my-8 sm:w-full ${
                                isModalGrande ? 'w-full' : 'sm:max-w-lg'
                            }`}
                        ...
                        >
                            <div
                                className={` ${
                                    tienePadding ? 'px-4 pt-5 pb-4 sm:p-6 sm:pb-4 lg:p-8' : 'p-0'
                                }`}
                            >
                                ...
                                <div className='flex items-end justify-end gap-4'>
                                    {onCancel && (
                                        <div className='mt-2 flex justify-center'>
                                            <button
                                                type='button'
                                                className='px-4 py-2 rounded-md border border-light text-sm hover:bg-background-accent-auto'
                                                onClick={onCancel}
                                            >
                                                {cancelText || 'Cancelar'}
                                            </button>
